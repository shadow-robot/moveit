...
OUTPUTFOLDER=$1 #defqult sticked
...
for dir1 in */ ; do
		cd "$dir1" && pwd && \
				for dir2 in */ ; do
						cd "$dir2" && pwd && FILEARRAY=(*.mp4) && \
								ffmpeg -y -i "${FILEARRAY[0]}" -i "${FILEARRAY[1]}" \
								-filter_complex hstack -c:v libx264 -crf 0 \
								"${OUTPUTFOLDER}${FILEARRAY[0]}" && \
								cd ../ && pwd ;
				done && cd ../ && pwd ;
done
